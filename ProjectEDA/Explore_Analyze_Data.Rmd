...
* **No collateral required:** Borrowers from Prosper don’t need collateral in 
...
colour = "white") +
# us loans
geom_jitter(data=loanData,aes(x=state.x, y=state.y, group=NULL), 
...
loanData.grp_borrower_state <- rename(loanData.grp_borrower_state, 
                                      c("tolower(state.name)" = "state.name"))

loanData.grp_borrower_state <- loanData.grp_borrower_state[1:5,]


writeLines("td, th { padding : 6px } th { background-color : brown ; 
           color : white; border : 1px solid white; } 
           td { color : brown ; border : 1px solid brown }", 
           con = "mystyle.css")
...
loanData.grp_borrower_state <-arrange(loanData.grp_borrower_state, 
                                      desc(LoanOriginalAmount_mean))
...
qplot(data = loanData, x = ListingCreationYear, color = I('black'), 
      fill = I('#00ccff'))

ggplot(aes(x = ListingCreationYear , y = LoanOriginalAmount ), 
       data = loanData) + 
...
qplot(data = loanData, x = ListingCreationMonth, color = I('black'), 
      fill = I('#00ccff')) + 
...
qplot(data = loanData, x = ListingCreationDay, color = I('black'), 
      fill = I('#00ccff')) + 
...
qplot(data = loanData, x = ListingCategory..numeric., binwidth = 1, 
      color = I('black'), fill = I('#00ccff')) + 
...
with(loanData, cor.test(LoanOriginalAmount, MonthlyLoanPayment, 
                        method = 'pearson'))
...
ggplot(aes(x = EstimatedReturn, y = Investors), 
       data = subset(loanData,!is.na(EstimatedReturn) & !is.na(Investors))) +
  geom_jitter(alpha = 1/20) +
  geom_smooth(method = 'lm', color = 'red') +
...
ggplot(aes(x = LoanOriginalAmount, y = EmploymentStatusDuration), 
       data = loanData) +
  geom_jitter(alpha = 1/20) +
  geom_smooth(method = 'lm', color = 'red') +
...
# Get correlation
with(loanData, cor.test(LoanOriginalAmount, EmploymentStatusDuration, 
                        method = 'pearson'))
```
...
# Get correlation
with(loanData, cor.test(LoanOriginalAmount, BankcardUtilization, 
                        method = 'pearson'))
...
# Change column names
names(loandData.numeric)[1:length(loandData.numeric)] <- 
  sprintf("x%d", 1:length(loandData.numeric))
...
corrplot(loandData.numeric.cor, method = "square", is.corr = FALSE, type="full", 
         tl.cex = 0.5, diag = FALSE, tl.offset = 1, cl.length = 20, 
...
with(loanData, cor.test(LoanOriginalAmount, LP_ServiceFees, 
                        method = 'pearson'))
...
ggplot(data=loanData, aes(loanData$ProsperRating..Alpha., 
                          loanData$EstimatedLoss)) + 
  geom_boxplot() 

ggplot(data=loanData, aes(ProsperRating..Alpha., EstimatedLoss)) + 
...
# Strong positive correlation
with(loanData, cor.test(ProsperRating..numeric., EstimatedLoss, 
                        method = 'pearson'))
...
ggplot(aes(x = EmploymentStatusDuration, y = MonthlyLoanPayment_mean), 
       data = loanData.grp_term_empStatus) + geom_line(aes(color = Term )) +
...
ggplot(aes(x = ListingCategory..numeric., y = MonthlyLoanPayment_mean, fill=Term), 
       data = loanData.grp_term_ListingCategory) + 
   geom_bar(position = "stack", stat = "identity") +
    xlim(0, quantile(loanData$ListingCategory..numeric., 0.95, na.rm = TRUE))  + 
   xlab("Listing Category") + 
   ylab("Monthly Loan Payment - USD") +
   labs(title = "Listing Category vs Monthly Payment", fill = "Term - months") 
...
scale_fill_distiller(palette = "Spectral") +
theme_bw() + 
labs(title="Number of Loans per State", fill = "Loan Original Mean - Number")
...
scale_fill_distiller(palette = "Spectral") +
theme_bw() + 
labs(title = "Amount of money per State", fill = "Loan Original Mean - USD.")
...
ggplot(aes(x = ListingCategory..numeric., y = MonthlyLoanPayment_mean), 
       data = loanData.grp_term_ListingCategory) + geom_line(aes(color = Term ), 
                                                             stat = "summary", 
                                                             fun.y = median) +
   xlim(0, quantile(loanData$ListingCategory..numeric., 0.95, na.rm = TRUE))  + 
   xlab("Listing Category") + 
   ylab("Monthly Loan Payment - USD") +
   labs(title = "Listing Category vs Monthly Payment", color = "Term - months") 
